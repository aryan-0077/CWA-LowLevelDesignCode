...
public class RateLimiterFactory {
    ...
    private static final Map<RateLimiterType, Function<Map<String, Object>, IRateLimiter>> RATE_LIMITER_FACTORIES = new HashMap<>();
    ...
    static {
        ...
        RATE_LIMITER_FACTORIES.put(RateLimiterType.TOKEN_BUCKET, config -> {
            ...
            // Support both the original tokensPerSecond (double) and the new refreshRate (int)
            if (config.containsKey("refreshRate")) {
                int refreshRate = (int) config.get("refreshRate");
                return new TokenBucketStrategy(capacity, refreshRate);
            } else {
                // For backward compatibility with the original implementation
                // Convert double tokensPerSecond to int refreshRate
                double tokensPerSecond = (double) config.getOrDefault("tokensPerSecond", 10.0);
                int refreshRate = (int) Math.round(tokensPerSecond);
                return new TokenBucketStrategy(capacity, refreshRate);
            }
        });
    ...
    /**
     * New simplified factory method for token bucket using refreshRate
     */
    public static IRateLimiter createTokenBucket(int capacity, int refreshRate) {
        Map<String, Object> config = new HashMap<>();
        config.put("capacity", capacity);
        config.put("refreshRate", refreshRate);
        return createLimiter(RateLimiterType.TOKEN_BUCKET, config);
    }
